<script lang="ts">
	...
	function trendText(cur: number, prev: number) {
		if (prev > cur) return { text: `↑${prev - cur}`, color: 'var(--success)' };
		if (prev < cur) return { text: `↓${cur - prev}`, color: 'var(--error)' };
		return { text: '—', color: 'var(--text-tertiary)' };
	}
</script>

<div class="min-h-screen animate-fadeIn px-4 md:px-6 pt-4 md:pt-6 pb-12">
	<h1 class="text-[20px] font-semibold text-[var(--text-primary)] tracking-tight mb-5">Leaderboards</h1>

	<!-- Tabs + Controls -->
	<div class="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-5">
		<div class="flex gap-1 overflow-x-auto">
			{#each [{ id: 'networks', label: 'Projects' }, { id: 'earners', label: 'Earners' }, { id: 'uptime', label: 'Uptime' }] as t}
				<button
					onclick={() => (tab = t.id as Tab)}
					class="inline-flex items-center h-[32px] px-4 rounded-[5px] text-[12px] font-medium border-none cursor-pointer transition-colors whitespace-nowrap {tab === t.id ? 'bg-[var(--accent-subtle)] text-[var(--text-accent)]' : 'bg-[var(--surface-1)] text-[var(--text-secondary)] hover:bg-[var(--surface-2)]'}"
				>{t.label}</button>
			{/each}
		</div>
	...
	<div class="rounded-[8px] border border-[var(--border-default)] bg-[var(--surface-1)] overflow-x-auto" style="-webkit-overflow-scrolling:touch">
		...
		{#if tab === 'networks'}
			...
			{#each topNetworks as app, i}
				...
				{@const tr = trendText(rank, pseudoTrend(app.id, rank))}
				<a href="/apps/{app.id}" class="grid items-center px-4 py-3 border-b border-[var(--border-default)] hover:bg-[var(--surface-2)] transition-colors no-underline"
					...
					<span class="flex justify-end text-[10px] font-semibold" style="color:{tr.color}">{tr.text}</span>
				</a>
			{/each}
		...
		{#if tab === 'earners'}
			<div class="grid items-center px-4 py-2 text-[10px] font-semibold uppercase tracking-wide text-[var(--text-tertiary)] border-b border-[var(--border-default)]" style="grid-template-columns:1fr 130px 80px 80px 50px; gap:0 16px">
				<span>#  Miner</span><span class="text-right">Earned</span><span class="text-right">Projects</span><span class="text-right">Uptime</span><span class="text-right">Trend</span>
			</div>
			{#each topEarners as miner, i}
				{@const rank = i + 1}
				{@const color = RANK_COLORS[rank] ?? 'var(--text-tertiary)'}
				{@const tr = trendText(rank, pseudoTrend(miner.minerId, rank))}
		...
		{#if tab === 'uptime'}
			...
			{#each bestUptime as miner, i}
				{@const rank = i + 1}
				{@const color = RANK_COLORS[rank] ?? 'var(--text-tertiary)'}
				{@const tr = trendText(rank, pseudoTrend(miner.minerId + 'uptime', rank))}
