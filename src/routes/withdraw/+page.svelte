<script>
  ...
  const payouts = $derived(
    minerId ? ($backendState.payouts ?? []).filter((p) => p.minerId === minerId) : []
  );

  const totalEarned = $derived(
    payouts.reduce((sum, p) => sum + (p.minerAmount ?? p.gross ?? 0), 0)
  );
  ...
  const canWithdraw = $derived(
    !submitting && amount && Number(amount) > 0 && Number(amount) <= availableBalance
  );
