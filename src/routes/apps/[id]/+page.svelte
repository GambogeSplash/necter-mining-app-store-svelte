<script>
  ...
  const privacyLabels = {
    dataCollected: [
      { type: 'Hardware Specs', purpose: 'Task Assignment', icon: 'cpu' },
      { type: 'Network Stats', purpose: 'Performance Monitoring', icon: 'wifi' },
      { type: 'Location (Region)', purpose: 'Network Optimization', icon: 'map-pin' },
      { type: 'Wallet Address', purpose: 'Rewards Distribution', icon: 'database' },
    ],
  ...
  const attCaps = $derived(minerId ? $backendState.attestationCapabilitiesByMinerId?.[minerId] ?? null : null);
  ...
  const derivedStatus = $derived(
    existingSubscription
      ? existingSubscription.status === 'active' ? 'mining' : 'subscribed'
      : 'not-subscribed'
  );
  ...
  const stakeAmount = $derived(
    app?.slaRequirements?.slashingPenalty
      ? Math.max(0, Math.round((app.slaRequirements.slashingPenalty / 100) * 10000))
      : 0
  );

  // ─── Local state ─────────────────────────────────────────────────────────────
  let miningStatus = $state('not-subscribed');
  let miningStats = $state({
    tasksCompleted: 0,
    earnings: 0,
    uptime: 0,
    proofs: 0,
  ...
  function handleSubscribe() {
    ...
    if (existingSubscription) {
      // Would open wizard — for now just go to mining
    ...
    if (!sub) {
      alert(missingAttestations.length > 0
  ...
  const tabLabels = ['overview', 'economics', 'requirements', 'reviews', 'privacy', 'versions'];
</script>

{#if !app}
  <div class="min-h-screen flex items-center justify-center text-[var(--text-tertiary)] text-[13px]">
    App not found.
  </div>
{:else}
  <div class="min-h-screen pb-28 bg-[var(--surface-0)]">
    ...
    <div class="border-b border-[var(--border-default)] bg-[var(--surface-1)]">
      <div class="max-w-[960px] mx-auto px-6">
        <a
          href="/discover"
          class="inline-flex items-center gap-1 h-11 text-[13px] text-[var(--text-secondary)] no-underline"
        >
    ...
    <!-- ── Main content ── -->
    <div class="max-w-[960px] mx-auto px-6 pt-8 pb-0">

      <!-- ═══════════════════════════════════════════════════════
           HEADER — icon, name, developer, meta, CTA
      ════════════════════════════════════════════════════════ -->
      <div class="flex gap-5 items-start mb-7">
        <!-- App icon -->
        <div class="shrink-0">
          <img
            src={app.icon || appIconDataUri}
            alt={app.name}
            width="64"
            height="64"
            class="rounded-[12px] block"
          />
        </div>
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-start gap-3 flex-wrap">
            <div class="flex-1 min-w-0">
              <h1 class="text-[24px] font-semibold tracking-[-0.02em] text-[var(--text-primary)] m-0 leading-8">
                {app.name}
              </h1>
              <a href="/profiles/{app.developerAddress}" class="text-[13px] text-[var(--text-secondary)] hover:text-[var(--text-accent)] transition-colors duration-100 mt-0.5 block no-underline">
                {app.developer}
              </a>
              ...
              <div class="flex items-center gap-2 mt-2 flex-wrap">
                ...
                <div class="flex items-center gap-[3px]">
                  {#each Array(5) as _, i}
                    ...
                    />
                  {/each}
                  <span class="text-[12px] text-[var(--text-secondary)] font-mono ml-0.5">
                ...
                <!-- Category badge -->
                <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.accent}">
                  {app.category}
                </span>

                <!-- Trust score -->
                {#if trustScore}
                  <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses[getTrustVariant(trustScore.overallScore)]}">
                    {getTrustLabel(trustScore.overallScore)}
                  </span>
                {/if}

                <!-- Developer verification -->
                <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses[developerVerificationStatus === 'verified' ? 'success' : developerVerificationStatus === 'pending' ? 'warning' : 'neutral']}">
                  {developerVerificationStatus === 'verified' ? 'Verified Dev' : developerVerificationStatus === 'pending' ? 'Verification Pending' : 'Unverified'}
                </span>

                {#if listingStatus === 'beta'}
                  <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.warning}">Beta</span>
                {/if}
              </div>
            </div>
            ...
            <div class="flex items-center gap-[6px] shrink-0">
              {#if miningStatus === 'not-subscribed'}
                <button class="btn-subscribe font-semibold" onclick={handleSubscribe}>
                  Subscribe
                </button>
              {/if}
              {#if miningStatus === 'subscribed'}
                <button
                  class="inline-flex items-center gap-1 h-7 px-[10px] rounded-[5px] border border-[var(--success)] bg-[rgba(76,183,130,0.12)] text-[var(--success)] text-[12px] font-semibold cursor-pointer"
              ...
              {#if miningStatus === 'mining'}
                <button
                  class="inline-flex items-center gap-1 h-7 px-[10px] rounded-[5px] border border-[var(--error)] bg-[rgba(235,87,87,0.12)] text-[var(--error)] text-[12px] font-semibold cursor-pointer"
          ...
          <!-- Status hint -->
          <p class="text-[12px] text-[var(--text-tertiary)] mt-[10px]">
      ...
      <div class="grid grid-cols-5 gap-2 mb-8">
        ...
        <div class="bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[8px] px-4 py-[14px] flex flex-col gap-1">
          ...
          <p class="text-[16px] font-semibold text-[var(--text-primary)] font-mono [font-feature-settings:'tnum']">${app.avgEarningsPerDay.toFixed(2)}</p>
        ...
        <div class="bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[8px] px-4 py-[14px] flex flex-col gap-1">
          ...
          <p class="text-[16px] font-semibold text-[var(--text-primary)] font-mono [font-feature-settings:'tnum']">{stakeAmount > 0 ? `${stakeAmount.toLocaleString()} NCT` : 'None'}</p>
        ...
        <div class="bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[8px] px-4 py-[14px] flex flex-col gap-1">
          ...
          <p class="text-[16px] font-semibold text-[var(--text-primary)] font-mono [font-feature-settings:'tnum']">{app.totalMiners >= 1000 ? `${(app.totalMiners / 1000).toFixed(1)}K` : app.totalMiners}</p>
        ...
        <div class="bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[8px] px-4 py-[14px] flex flex-col gap-1">
          ...
          <p class="text-[16px] font-semibold text-[var(--text-primary)] font-mono [font-feature-settings:'tnum']">{app.slaRequirements?.minUptime ?? 95}%</p>
        ...
        <div class="bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[8px] px-4 py-[14px] flex flex-col gap-1">
          ...
          <p class="text-[16px] font-semibold text-[var(--text-primary)] font-mono [font-feature-settings:'tnum']">{app.attestations}</p>
      ...
      {#if miningStatus !== 'not-subscribed'}
        <div
          class="rounded-[8px] px-5 py-4 mb-7"
          style="background: {miningStatus === 'mining' ? 'rgba(76,183,130,0.06)' : 'rgba(242,153,74,0.06)'}; border: 1px solid {miningStatus === 'mining' ? 'rgba(76,183,130,0.20)' : 'rgba(242,153,74,0.20)'};"
        >
          <!-- Status header -->
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
              <span class={miningStatus === 'mining' ? 'status-dot status-dot-active' : 'status-dot status-dot-proving'} />
              <span
            ...
            <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses[miningStatus === 'mining' ? 'success' : 'warning']}">
          ...
          <div class="flex gap-2 flex-wrap">
            <a href="/mining" class="inline-flex items-center h-7 px-3 rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[12px] no-underline">
              Mining Dashboard
            </a>
            <a href="/mining?tab=proofs" class="inline-flex items-center h-7 px-3 rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[12px] no-underline">
              Proof Queue
            </a>
            <a href="/mining/earnings" class="inline-flex items-center h-7 px-3 rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[12px] no-underline">
              Earnings
            </a>
            {#if miningStatus === 'subscribed'}
              <button
                onclick={handleStartMining}
                class="inline-flex items-center gap-1 h-7 px-3 rounded-[5px] bg-[var(--success)] text-[#0C0C0E] text-[12px] font-semibold cursor-pointer border-0"
              >
                <Play size={11} />
                Start jobs
              </button>
      ...
      <!-- ═══════════════════════════════════════════════════════
           ATTESTATION ALERT
      ════════════════════════════════════════════════════════ -->
      <div class="flex items-start gap-[10px] bg-[rgba(255,191,0,0.06)] border border-[var(--border-accent)] rounded-[8px] px-[14px] py-3 mb-8">
        ...
        <div>
          <p class="text-[13px] text-[var(--text-primary)]">
            ...
            <span class="text-[var(--text-secondary)]">
              {app.attestations} on-chain attestations from governance members
            </span>
          </p>
          {#if attestationReqs.length > 0}
            <p class="text-[12px] text-[var(--text-secondary)] mt-1">
              ...
              {#if missingAttestations.length > 0}
                <span class="text-[var(--warning)] ml-[6px]">
                  (missing: {missingAttestations.join(', ')})
                </span>
              {/if}
            </p>
          {/if}
      ...
      <!-- ═══════════════════════════════════════════════════════
           TABS
      ════════════════════════════════════════════════════════ -->
      <div class="flex gap-0 bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-[3px] mb-6 w-fit">
        {#each tabLabels as tab}
          <button
            class="h-7 px-3 rounded-[5px] text-[12px] font-medium cursor-pointer border-0 transition-colors duration-100
              {activeTab === tab
      ...
      {#if activeTab === 'overview'}
        <div class="grid grid-cols-2 gap-4">
          <!-- Description -->
          <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-5 col-span-2">
            <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">Description</p>
            <p class="text-[13px] text-[var(--text-secondary)] leading-5 m-0">{app.description}</p>
          </div>

          <!-- Network Features -->
          <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-5">
            <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">Network Features</p>
            <ul class="list-none p-0 m-0 flex flex-col gap-[10px]">
              {#each (app.features ?? []).slice(0, 4) as feature}
                <li class="flex items-start gap-2 text-[13px] text-[var(--text-secondary)]">
                  <CheckCircle2 size={13} strokeWidth={1.5} class="text-[var(--success)] shrink-0 mt-[2px]" />
                  {feature}
                </li>
              {/each}
            </ul>
            ...
            <div class="flex flex-col gap-[10px]">
              {#each [
                ...
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-[13px] text-[var(--text-primary)] font-medium">{row.value}</span>
                </div>
              {/each}
            </div>
          </div>
            ...
            <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-5 col-span-2">
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2">
                  ...
                  <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-0">Trust Score</p>
                </div>
                <div class="flex items-center gap-2">
                  <span class="text-[20px] font-semibold font-mono text-[var(--text-primary)]">
                    {trustScore.overallScore}<span class="text-[13px] text-[var(--text-tertiary)] font-normal">/100</span>
                  </span>
                  {#if trustScore.trendingDirection === 'improving'}
                    <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.success}">Improving</span>
                  {/if}
                  {#if trustScore.trendingDirection === 'declining'}
                    <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.error}">Declining</span>
                  {/if}
                </div>
              </div>
                ...
                {#each [
                  ...
                  <div>
                    <div class="flex justify-between mb-[6px]">
                      ...
                      <span class="text-[12px] font-mono text-[var(--text-primary)] font-medium">{item.value}%</span>
                    </div>
                    <div class="h-[3px] bg-[var(--surface-3)] rounded-full overflow-hidden">
                      <div
                        class="h-full rounded-full"
                        style="width: {item.value}%; background: {item.value >= 80 ? 'var(--success)' : item.value >= 60 ? 'var(--warning)' : 'var(--error)'};"
                      />
                    </div>
                  </div>
                {/each}
              ...
              {#if trustScore.redFlags.length > 0}
                <div class="mt-4 bg-[rgba(235,87,87,0.08)] border border-[rgba(235,87,87,0.20)] rounded-[6px] px-3 py-[10px]">
                  <p class="text-[11px] font-semibold text-[var(--error)] mb-[6px] uppercase tracking-[0.04em]">Warnings</p>
      ...
      {#if activeTab === 'economics'}
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
              {#each [
                ...
                ...(baseRewardPerTask != null
                  ? [{ label: 'Per-Task Reward', value: `${baseRewardPerTask.toFixed(4)} ${app.rewardToken}${minRewardPerTask != null && maxRewardPerTask != null ? ` (${minRewardPerTask.toFixed(4)}–${maxRewardPerTask.toFixed(4)})` : ''}` }]
                  : []),
                ...
                { label: 'Monthly Average', value: `$${(app.avgEarningsPerDay * 30).toFixed(2)} / miner` },
              ] as row}
                <div class="flex justify-between items-center py-2 border-b border-[var(--border-default)]">
                  <span class="text-[13px] text-[var(--text-tertiary)]">{row.label}</span>
                  <span class="text-[13px] text-[var(--text-primary)] font-mono [font-feature-settings:'tnum']">{row.value}</span>
                </div>
              {/each}
            </div>
          ...
          <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-4">
            <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">Reward Tiers</p>
            ...
            <div class="flex flex-col">
              {#each [
                { tier: 'Tier 1', range: 'Top 10%', multiplier: '1.5x', color: 'var(--text-accent)', bg: 'var(--accent-subtle)' },
                { tier: 'Tier 2', range: 'Top 25%', multiplier: '1.25x', color: 'var(--success)', bg: 'rgba(76,183,130,0.12)' },
                { tier: 'Tier 3', range: 'Top 50%', multiplier: '1.0x', color: 'var(--text-primary)', bg: 'var(--surface-3)' },
                { tier: 'Tier 4', range: 'Bottom 50%', multiplier: '0.85x', color: 'var(--text-secondary)', bg: 'var(--surface-3)' },
              ] as t, i}
                <div class="flex items-center justify-between py-[10px] {i < 3 ? 'border-b border-[var(--border-default)]' : ''}">
                  ...
                  <div class="flex items-center gap-4">
                    <span class="text-[13px] font-semibold font-mono [font-feature-settings:'tnum']" style="color: {t.color};">
                      {t.multiplier}
                    </span>
                    <span class="text-[12px] text-[var(--text-tertiary)] font-mono [font-feature-settings:'tnum'] w-[90px] text-right">
          ...
          <!-- SLA -->
          <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-5">
            <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">SLA &amp; Penalties</p>
            <div class="flex flex-col gap-2">
              {#each [
                { label: 'Minimum Uptime Required', value: `${app.slaRequirements?.minUptime || 95}%` },
                { label: 'Slashing Penalty', value: `${app.slaRequirements?.slashingPenalty || 5}%` },
        ...
        <div class="flex flex-col gap-4">
          <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-5">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div class="flex items-start gap-[10px] bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[8px] px-[14px] py-3">
                ...
                <div>
                  ...
                  <p class="text-[12px] text-[var(--text-tertiary)] mt-0.5">{app.requirements.storage || '100 GB'}</p>
                </div>
              </div>
              <!-- Bandwidth -->
              <div class="flex items-start gap-[10px] bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[8px] px-[14px] py-3">
                ...
                <div>
                  ...
                  <p class="text-[12px] text-[var(--text-tertiary)] mt-0.5">{app.requirements.bandwidth || '10 Mbps'}</p>
                </div>
              </div>
              <!-- GPU (conditional) -->
            ...
            <a
              ...
              class="inline-flex items-center gap-1 mt-4 h-8 px-[14px] rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[13px] no-underline"
            >
        ...
        <div class="grid gap-4" style="grid-template-columns: 220px 1fr;">
              ...
              {#each ratingDistribution as item}
                <div class="flex items-center gap-[6px]">
                  ...
                  <div class="flex-1 h-[3px] bg-[var(--surface-3)] rounded-full overflow-hidden">
                    <div class="h-full bg-[var(--accent-base)] rounded-full" style="width: {item.percentage}%;" />
                  </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-3">
              ...
              <select class="h-7 px-2 rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[12px] cursor-pointer">
            ...
            <div class="flex flex-col gap-2">
              {#each (app.reviews ?? []).slice(0, 5) as review}
                <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] px-4 py-[14px]">
                  <div class="flex items-start justify-between mb-2">
                    <div>
                      <div class="flex items-center gap-2">
                        <img src={minerAvatarDataUri(review.minerId)} alt="" class="w-[22px] h-[22px] rounded-[5px]" />
                        <p class="text-[13px] font-semibold text-[var(--text-primary)]">{review.minerUsername}</p>
                        <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.neutral}">Verified Miner</span>
                      </div>
                  ...
                  <p class="text-[13px] font-semibold text-[var(--text-primary)] mb-1">Great mining experience</p>
                  <p class="text-[13px] text-[var(--text-secondary)] leading-5 mb-[10px]">{review.comment}</p>
                  <div class="flex gap-4">
                    <button type="button" class="inline-flex items-center gap-1 text-[12px] text-[var(--text-tertiary)] bg-transparent border-0 cursor-pointer p-0">
              ...
              <a
                ...
                class="inline-flex items-center justify-center gap-[6px] h-8 px-4 rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[13px] no-underline mt-1"
              >
      ...
      {#if activeTab === 'privacy'}
        <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-6">
          <div class="flex items-center gap-2 mb-[6px]">
            ...
            <h3 class="text-[14px] font-semibold text-[var(--text-primary)] m-0">Attestation &amp; Privacy</h3>
          </div>
          <p class="text-[13px] text-[var(--text-secondary)] mb-6 leading-5">
            The developer indicated that this app's privacy practices may include handling of data as described below.
          </p>

          <!-- Data Used to Track You -->
          <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">Data Used to Track You</p>
          <div class="grid grid-cols-2 gap-2 mb-6">
            {#each privacyLabels.dataCollected as item}
              <div class="flex items-start gap-[10px] bg-[var(--surface-2)] border border-[var(--border-default)] rounded-[8px] px-[14px] py-3">
                {#if item.icon === 'cpu'}
                  <Cpu size={14} strokeWidth={1.5} class="text-[var(--text-tertiary)] shrink-0 mt-[1px]" />
                {:else if item.icon === 'wifi'}
                  <Wifi size={14} strokeWidth={1.5} class="text-[var(--text-tertiary)] shrink-0 mt-[1px]" />
                {:else if item.icon === 'map-pin'}
                  <MapPin size={14} strokeWidth={1.5} class="text-[var(--text-tertiary)] shrink-0 mt-[1px]" />
                {:else if item.icon === 'database'}
                  <Database size={14} strokeWidth={1.5} class="text-[var(--text-tertiary)] shrink-0 mt-[1px]" />
                {/if}
                <div>
          ...
          <!-- Data Linked to You -->
          <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">Data Linked to You</p>
          <div class="flex flex-wrap gap-[6px] mb-6">
            {#each privacyLabels.dataLinkedToYou as item}
              <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.warning}">
          ...
          <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">Data Not Linked to You</p>
          <div class="flex flex-wrap gap-[6px] mb-6">
            {#each privacyLabels.dataNotLinkedToYou as item}
              <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.neutral}">{item}</span>
            {/each}
          </div>
          ...
          <!-- Data Not Collected -->
          <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">Data Not Collected</p>
          <div class="flex flex-wrap gap-[6px]">
            {#each privacyLabels.dataNotCollected as item}
              <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.success}">
      ...
      {#if activeTab === 'versions'}
        <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-6">
          <div class="flex items-center gap-2 mb-5">
            ...
            <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-0">Version History</p>
          </div>

          <div class="relative">
            <!-- Timeline line -->
            <div class="absolute bg-[var(--border-default)] left-[7px] top-[8px] bottom-[8px] w-px" />
              ...
              {#each versionHistory as version, index}
                <div class="pl-7 relative">
                  ...
                  <div
                    ...
                    style="left: 0; top: 4px; width: 15px; height: 15px; background: {index === 0 ? 'var(--accent-base)' : 'var(--surface-3)'}; border: 2px solid {index === 0 ? 'var(--accent-base)' : 'var(--border-strong)'};"
                  />

                  <div class="flex items-center gap-[10px] mb-[6px]">
                    <span class="text-[13px] font-semibold text-[var(--text-primary)] font-mono">{version.version}</span>
                    <span class="text-[12px] text-[var(--text-tertiary)]">{version.date}</span>
                    {#if index === 0}
                      <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses.accent}">Latest</span>
                    {/if}
                  </div>
        ...
        <div class="flex gap-3 overflow-x-auto pb-1">
          {#each getAppScreenshots(app) as src, i}
            <img
              ...
              class="shrink-0 border border-[var(--border-default)] rounded-[12px] w-[440px] h-[280px]"
            />
          {/each}
        </div>
      </div>
      ...
      <!-- ═══════════════════════════════════════════════════════
           DEVELOPER INFO
      ════════════════════════════════════════════════════════ -->
      <div class="mt-8 bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-5">
        <p class="text-[10px] font-semibold tracking-[0.04em] uppercase text-[var(--text-tertiary)] mb-3">Developer</p>
        <div class="flex justify-between items-start">
          <div>
            <a href="/profiles/{app.developerAddress}" class="text-[14px] font-semibold text-[var(--text-primary)] hover:text-[var(--text-accent)] transition-colors duration-100 mb-[6px] block no-underline">
              {app.developer}
            </a>
            ...
            <span class="inline-flex items-center h-5 px-[6px] rounded-[3px] text-[11px] font-medium whitespace-nowrap {variantClasses[developerVerificationStatus === 'verified' ? 'success' : developerVerificationStatus === 'pending' ? 'warning' : 'neutral']}">
              {developerVerificationStatus === 'verified' ? 'Verified Developer' : developerVerificationStatus === 'pending' ? 'Verification Pending' : 'Unverified Developer'}
            </span>
          </div>
          <a
            href="#"
          ...
          <div class="grid grid-cols-4 gap-[10px]">
            {#each relatedApps as relatedApp}
              <a href="/apps/{relatedApp.id}" class="no-underline">
                <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[8px] p-3 cursor-pointer transition-[border-color] duration-100 ease-out hover:border-[var(--border-hover)]">
                  <div class="flex items-center gap-[10px] mb-[10px]">
                    <img
                      src={relatedApp.icon || appIconDataUri}
                      alt={relatedApp.name}
                      width="36"
                      height="36"
                    ...
                    />
                    <div class="min-w-0">
                      <p class="text-[13px] font-semibold text-[var(--text-primary)] overflow-hidden text-ellipsis whitespace-nowrap">{relatedApp.name}</p>
                      <p class="text-[11px] text-[var(--text-tertiary)] mt-[1px]">{relatedApp.category}</p>
                    </div>
                  </div>
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-[3px]">
                      <Star size={10} strokeWidth={0} fill="var(--accent-base)" />
                      <span class="text-[11px] text-[var(--text-secondary)] font-mono">{relatedApp.reputationScore}</span>
                    </div>
                    <span class="text-[11px] text-[var(--text-accent)] font-mono font-medium">${relatedApp.avgEarningsPerDay}/day</span>
                  </div>
                </div>
              </a>
            {/each}
    ...
    <!-- ═══════════════════════════════════════════════════════
         STICKY CTA BAR
    ════════════════════════════════════════════════════════ -->
    <div class="fixed bottom-0 left-0 right-0 z-50 border-t border-[var(--border-default)] bg-[var(--surface-1)]/95 backdrop-blur-sm">
      <div class="max-w-[960px] mx-auto px-6 h-14 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <img src={app.icon || appIconDataUri} alt="" width="28" height="28" class="rounded-[6px]" />
          <div>
    ...
    {#if shareOpen}
      ...
      <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50" onclick={() => shareOpen = false}>
        <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[12px] p-6 w-[420px] max-w-[90vw] shadow-xl" onclick={(e) => e.stopPropagation()}>
          ...
          <div class="flex justify-end gap-2">
            <button onclick={handleShare} class="h-8 px-[14px] rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[13px] cursor-pointer">
              Share / Copy
            </button>
            <button onclick={handleCopyLink} class="h-8 px-[14px] rounded-[5px] bg-[var(--accent-base)] text-[#0C0C0E] text-[13px] font-semibold cursor-pointer border-0">
              Copy link
            </button>
            <a href={shareUrl} target="_blank" rel="noreferrer" class="inline-flex items-center h-8 px-[14px] rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[13px] no-underline">
              Open
            </a>
          </div>
        </div>
    ...
    {#if reportOpen}
      <!-- svelte-ignore a11y_click_events_have_key_events -->
      <!-- svelte-ignore a11y_no_static_element_interactions -->
      <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50" onclick={() => { reportOpen = false; reportReason = ''; }}>
        <div class="bg-[var(--surface-1)] border border-[var(--border-default)] rounded-[12px] p-6 w-[480px] max-w-[90vw] shadow-xl" onclick={(e) => e.stopPropagation()}>
          ...
          <div class="flex flex-col gap-[14px]">
            <!-- Category -->
            <div class="flex flex-col gap-[6px]">
              <p class="text-[11px] text-[var(--text-tertiary)] font-semibold tracking-[0.04em] uppercase">Category</p>
              <select
            ...
            <!-- Severity -->
            <div class="flex flex-col gap-[6px]">
              <p class="text-[11px] text-[var(--text-tertiary)] font-semibold tracking-[0.04em] uppercase">Severity</p>
              <select
            ...
            <!-- Reason -->
            <div class="flex flex-col gap-[6px]">
              <p class="text-[11px] text-[var(--text-tertiary)] font-semibold tracking-[0.04em] uppercase">Reason</p>
              <textarea
          ...
          <div class="flex justify-end gap-2 mt-4">
            <button onclick={() => reportOpen = false} class="h-8 px-[14px] rounded-[5px] border border-[var(--border-default)] bg-[var(--surface-2)] text-[var(--text-secondary)] text-[13px] cursor-pointer">
              Cancel
            </button>
            <button
